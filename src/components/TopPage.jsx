...
export const TopPage = () => {
  ...
  const heroColor = useRef();
  const heroFruits = useRef();

  const tl = gsap.timeline({ repeatDelay: 0.8, repeat: -1 });
  const q = gsap.utils.selector(heroFruits);
  ...
  useEffect(() => {
    document.addEventListener('mousemove', (e) => {
      if (heroColor.current) {
        const mouseX = e.clientX;
        const mouseY = e.clientY;
        heroColor.current && gsap.to(heroColor.current, {
          x: mouseX,
          y: mouseY,
          duration: 2
        });
  ...
  return (
    <Body className="body-container" label={label}>
      <TitleEn>Web Design and Front End Web Development</TitleEn>
      <Hero>
        <div className="hero-color" ref={heroColor}></div>
      ...
      <FruitsList onMouseEnter={onMouseEnterAction} onMouseLeave={onMouseLeaveAction} ref={heroFruits}>
